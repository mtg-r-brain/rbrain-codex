...
Every service container SHALL declare a healthcheck probing its `GET /health` endpoint (per `repository-conventions`), and SHALL gate its start on the healthy state of the infrastructure it requires (`depends_on` with `condition: service_healthy` on PostgreSQL and, where applicable, NATS). The stack SHALL NOT rely on restart loops as the ordering mechanism.
