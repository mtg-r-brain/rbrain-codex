...
- [x] 1.4 Wire the catalog validation into `rbrain-codex` CI so any modification to `catalog.yaml` is checked on push.
...
- [x] 2.4 Add a CI check that any subject pattern declared in any future `OWNERSHIP.yaml.publishes` matches the convention regex.
...
- [x] 4.3 Archive this change via `openspec archive platform-architecture` once tasks 1.x–3.x are complete and CI is green.
