...
`oracle`'s budget covers in-process embedding inference (quantized 384-dim sentence model via ONNX): measured ~102 MiB serving steady state and ~115 MiB during sync-time corpus indexation (spike 2026-07-06), plus HTTP/DB baseline and margin.

`cortex`'s budget reflects live measurement (93 MiB steady state under the full agent loop, 2026-07-06); it remains the largest service allocation.
...
The sum of all per-context `max_rss_mb` values plus the external dependency budgets (PostgreSQL: 256 MB, Redis: 50 MB, NATS: 40 MB) SHALL NOT exceed 1024 MB for a single-node deployment. The current sum is 998 MB, leaving 26 MB of headroom.
